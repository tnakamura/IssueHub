...
{
    public sealed class AssignedIssuesPageViewModel : PageViewModel
    {
        public IStore<AppState> Store { get; }

        public AssignedIssuesPageViewModel(IStore<AppState> store)
            : base()
        {
            Store = store;
            PageTitle = R.Assigned;
        ...
        async Task InitializeAsync()
        {
            try
            {
                ...
                await Store.DispatchAsync(
                    IssueActions.LoadAssignedIssues(
                        Store.State.Issues.ShowAll));
            }
            finally
            {
